...
ts 中数组存储的所有元素类型必须一致，所以在标注某一变量的类型为数组时也需要标注数组中存储元素的类型。
...
当一个函数永远不可能执行到 `return` 的时候，返回值使用 `never`，常见常见为**抛出错误**
...
- 一个变量未声明切未标注的情况下，默认类型为 `any`
- 任何类型都可以赋值给 `any`
- `any` 可以赋值给任何类型
- `any` 类型有任意属性和方法
- 当 `"noImplicitAny" : true` 时，当无法对端一个变量的时候就会报错（或者只能推断为 any 类型），你需要通过显示的类型注解来消除这个错误
...
交叉类型也可以称为合并类型，把多个类型合并成一个新类型，结果类似于前文接口的合并，各个标注类型质检室且的关系
